%% Description
% This will show the 25 transect profile one by  one.
% Modified CEE199 final script for RA assignment. The backshore high
% elevation part (near houses) are excluded
...
% Preallocate for data, 17 DEMs all have 25 lines

packx = NaN(17,25,2500);   % Pack for cross-shore distance from -8isobath
packz = NaN(17,25,2500);   % Pack for corresponding elevation
packt = zeros(17,25);   % Pack for counting the length of data pairs
...
F    = getframe(FigH);
imwrite(F.cdata, ['P_The Beach pofiles at Transect',num2str(n),'.jpg'], 'jpg')
close all;
end
