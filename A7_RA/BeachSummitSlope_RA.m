...
for c = 1:17   % 17 dem datasets
    ...
    for n = 1 : 25   % 25 crosssections
        ...
        if ~(n == 16 || n == 17 || n >=22)
            % Exclude backshore part
            if n == 4 || n == 13
                packz(c,n,round(0.97*packt(c,n)):packt(c,n)) = NaN;
            else
                packz(c,n,round(0.98*packt(c,n)):packt(c,n)) = NaN;
            end
        end
...
for n = 1:25   % 25 crosssections
    ...
    for c = 1 : 17   % 17 dem datasets
        ...
        if c == 1 && n == 2
            [~,i]= nanmin(abs(squeeze(packz(c,n,:))-2));
        else if  c == 14 && n == 21
                [~,i]= nanmin(abs(squeeze(packz(c,n,:))-3));
            else
        ...
        if c ==1 && n == 3
            [~,i]= nanmin(abs(squeeze(packz(c,n,:))-3.313));
        end
        
        if c ==1 && n == 4
            [~,i]= nanmin(abs(squeeze(packz(c,n,:))-3.136));
        end
...
% Temporally averaged slope and maximum crest elevation
mbc = nanmean(max_BE);
mbc_2 = nanmean(max_BE_2);
mbs = nanmean(slope_b); % slope~ 1:mbs ~ x:z

% Mean slope and maximum crest among 17*25 value
t_avg_bs = nanmean(mbs);
t_avg_bc = nanmean(mbc);
t_avg_bc_2 = nanmean(mbc_2);
...
erb_maxBE_2 = max(max_BE_2) - mbc_2;
erb_minBE_2 = min(max_BE_2) - mbc_2;
...
yyaxis left
plot(x_ab,mbc,'-b','LineWidth',1.5)
errorbar(x_ab,mbc,erb_minBE,erb_maxBE,'b')

% Plot the maximum beach elevation
% plot(x_ab,nanmean(max_BE_2),'--','LineWidth',1.5)

ylabel('Maximum Elevation(m NAVD88)')
...
yyaxis right
plot(x_ab,mbs,'-r','LineWidth',1.5)
errorbar(x_ab,mbs,erb_minBS,erb_maxBS,'r')

xlabel('Transect Number')
ylabel('Foreshore Slope (Rise:Run)')
title('Mean Slope and Maximum Foreshore Elevation at Different Transects')
set(gca,'FontSize',20)
legend({'Elevation','Deviation of Elevation','Slope','Deviation of Slope'},'Location','NorthEast','FontSize',15)
...
yyaxis left
plot(x_ab,mbc_2,'-b','LineWidth',1.5)
errorbar(x_ab,mbc_2,erb_minBE_2,erb_maxBE_2,'b')
...
xlabel('Transect Number')
ylabel('Foreshore Slope')
title('Averaged Foreshore Slope and Maximum Elevation at Different Transects')
legend({'Elevation','Deviation of Elevation','Slope','Deviation of Slope'},'Location','NorthEast','FontSize',15)
set(gca,'FontSize',20)
% legend('Elevation','e','Slope','e','Location','NorthEast')
% xlim([-50,2450])
...
%% Export maximum elevation and foreshore slope
% csvwrite('Maximum_Elevation.csv',max_BE_2);
% csvwrite('Foreshore_Slope.csv',slope_b);
