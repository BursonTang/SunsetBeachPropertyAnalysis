...
% The only difference between this and "TransectProfilePlot_final" is that in
...
errorbar(x_ab,nanmean(dmx_msl,1),maxmsl,minmsl,'r')

plot(x_ab, nanmean(dmx_mhhw,1),'-b','LineWidth',1.5);
errorbar(x_ab,nanmean(dmx_mhhw,1),maxmhhw,minmhhw,'b')
...
%% Save profile data
% save Profile_Sunset.mat packx packz packt definition
% definition = table('packx','Distance from -8 meter isobath in meter','packz','Elevation in meter (NAVD88)', 'packt', 'The number of valid data pairs for each profile','P.S.','Row# => DEM#; Col# => Transects#; There are 17*25 profiles in total')